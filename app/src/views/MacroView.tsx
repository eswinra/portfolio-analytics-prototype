...
import { buildLens, dominantFactor, SENSITIVITIES, type LensRow } from '../lib/macro/lens';
...
const SECTIONS: [id: string, label: string][] = [
  ['mac-read', 'Where things stand'],
  ['mac-regime', 'Regime'],
  ['mac-lens', 'Portfolio lens'],
  ['mac-factors', 'Factors'],
  ['mac-board', 'Indicators'],
  ['mac-history', 'History'],
  ['mac-curve', 'Curve and channels'],
  ['mac-sources', 'Sources and method'],
];
...
export function MacroView() {
  ...
  const lens = useMemo(() => buildLens(MODEL, pub.pol), [pub]);
  ...
  const fund = lens[0]!;
  const selected = lens.find((r) => r.id === lensSel) ?? fund;
  const assets = lens.filter((r) => r.kind === 'asset');
  const lowest = assets.reduce((a, b) => ((b.exposure ?? 0) < (a.exposure ?? 0) ? b : a));
  const highest = assets.reduce((a, b) => ((b.exposure ?? 0) > (a.exposure ?? 0) ? b : a));
  ...
  return (
    <>
      ...
      <DateLine
        dataThrough={`${THROUGH} for the monthly factors; each indicator shows its own latest date`}
        retrieved={`FRED (Federal Reserve Bank of St. Louis), retrieved ${RETRIEVED}`}
      />
      <div className="muted-note vintage-note" role="note">
        <strong>Market context, kept apart from portfolio performance.</strong> Public U.S. economic
        series from FRED, read against their own history. Nothing on this tab is a LACERA return, a
        forecast, or a recommendation. The portfolio lens applies stated sensitivity assumptions to
        the IPS policy targets for the {pub.label}; it does not use holdings. The snapshot is dated:
        the page does not update itself.
      </div>
      ...
      <div id="mac-read" className="grid-kpi">
        <Panel tight kicker={`Where the economy sits — ${THROUGH}`}>
          ...
          <div className="stat-sub">
            {LEVEL
              ? `${LEVEL.where}. Growth ${sigma(LEVEL.growthZ)}, inflation ${sigma(LEVEL.inflationZ)} from their long-run norms.`
        ...
        >
          <div className="stat-value smaller">{DIRECTION ? DIRECTION.label : 'Not available'}</div>
          <div className="stat-sub">
            {DIRECTION
              ? `Core PCE inflation ${DIRECTION.inflation} (${signed(DIRECTION.inflationDelta, 2)} pp over three months); labor ${DIRECTION.labor} (unemployment ${signed(DIRECTION.laborDelta, 2)} pp, payrolls ${signed(DIRECTION.payrolls, 0)}k a month).`
        ...
        <Panel tight kicker={`Portfolio lens — ${pub.label} policy targets`}>
          <div className="stat-value smaller">
            {fund.exposure === null ? '—' : signed(fund.exposure, 2)}
          </div>
          <div className="stat-sub">
            Below zero: today’s factor readings lean against this policy mix under the stated
            sensitivities. Largest driver: {fundDriver ? fundDriver.name : '—'}. Most negative
            sleeve: {lowest.name} ({signed(lowest.exposure ?? 0, 2)}); most positive: {highest.name}{' '}
            ({signed(highest.exposure ?? 0, 2)}).
          </div>
          <div className="stat-foot">
      ...
      <div className="kpi-provenance">
        <ClassBadge c="calculated" />
        <ClassBadge c="proxy_estimate" />
        <span>
          Readings are calculated from public series: z = (latest − mean) ÷ standard deviation since
          1995. The lens figure is a proxy estimate built on stated assumptions.
        </span>
        <SourceLine records={[FRED]} />
      </div>
      ...
      <div id="mac-regime" className="grid-panels mt">
        ...
        >
          {LEVEL ? (
            <div className="quad-wrap">
              <QuadrantChart
                near={NEAR_NORM}
                trail={LEVEL.trail.map((p) => ({ g: p.g, i: p.i, label: monthLabel(p.month) }))}
                labels={{
          ...
          <p className="panel-note">
            {LEVEL?.sentence} The shaded cross is the near-norm band (±{NEAR_NORM} σ), inside which
            no quadrant is named. The path uses today’s mean and standard deviation for every month,
            so earlier points are not what could have been computed at the time. Bottom-left is the{' '}
            {QUADRANT_NAME.contraction.toLowerCase()} quadrant.
          </p>
          <SourceLine records={[FRED]} />
        </Panel>
        ...
        <Panel
          ...
          sub={
            DIRECTION
              ? `Evaluated on ${monthLabel(DIRECTION.month)}, the latest month in which core PCE, unemployment and payrolls are all published`
              : undefined
          }
        >
          {DIRECTION ? (
            <div className="table-scroll" role="region" aria-label="Direction rules" tabIndex={0}>
                ...
                <tbody>
                  <tr>
                    <td>
                      ...
                      <span className="row-date show-sm">
                        rising above +{RULES.inflationBand} pp; easing below −{RULES.inflationBand}{' '}
                        pp
                      </span>
                    </td>
                    <td className="num">{signed(DIRECTION.inflationDelta, 2)} pp</td>
                    <td className="hide-sm">
                      rising above +{RULES.inflationBand} pp; easing below −{RULES.inflationBand} pp
                    </td>
                    <td>
                  ...
                  <tr>
                    ...
                    <td rowSpan={2} className="hide-sm">
                      softening at +{RULES.laborSoftening} pp or more, or payrolls below zero; firm
                      at +{RULES.laborFirm} pp or less with payrolls above zero
                    </td>
                    <td rowSpan={2}>
                  ...
                  <tr>
                    <td>
                      ...
                      <span className="row-date show-sm">
                        labor softening at +{RULES.laborSoftening} pp or more, or payrolls below
                        zero; firm at +{RULES.laborFirm} pp or less with payrolls above zero
                      </span>
                    </td>
          ...
          <p className="panel-note">
            “Mixed signals” whenever inflation is steady or labor is mixed; otherwise the two
            results are named together. The thresholds are analyst assumptions, stated so they can
            be challenged. This is not a recession model.
          </p>
          <SourceLine records={[FRED]} />
        </Panel>
      </div>
      ...
      >
        <div className="kpi-provenance" style={{ marginTop: 0, marginBottom: 8 }}>
          <ClassBadge c="proxy_estimate" />
          <span>
            Exposure = Σ (stated sensitivity × factor z). Sleeves are weighted by the IPS Table 1
            long-term targets, not actual holdings. Select a row to see its factor arithmetic.
          </span>
        </div>
        <div className="lens-layout">
          <div className="table-scroll" role="region" aria-label="Exposure by sleeve" tabIndex={0}>
            <table className="table lens-table">
              <caption>Exposure by policy sleeve, in factor-weighted standard deviations</caption>
              <thead>
                <tr>
                ...
                {lens.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className={`lens-${r.kind}${on ? ' is-selected' : ''}`}>
                      ...
                      <td className="num">{r.exposure === null ? '—' : signed(r.exposure, 2)}</td>
                      <td className="hide-sm">
                        <ZBar z={r.exposure} max={2} />
                      </td>
                      <td className="hide-sm">{d ? `${d.name} ${signed(d.contrib!, 2)}` : '—'}</td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        <details className="details-block">
          <summary>Stated sensitivities — all {Object.keys(SENSITIVITIES).length} rows</summary>
          <div
          ...
          <p className="footnote">
            +1 = the class tends to benefit when the factor rises; −1 = it tends to suffer. Analyst
            priors, not estimated from LACERA returns. † Added so every IPS Table 1 sleeve has a
            row. Credit conditions: higher = tighter, so a negative sensitivity means tighter
            conditions hurt.
          </p>
        </details>
        <ul className="limits">
          <li>
            Not a forecast: a reading describes how today’s conditions line up with stated
            assumptions, not what any sleeve will return.
          </li>
          <li>
            Real rates carry large negative sensitivities in most sleeves, so a high real-rate
            reading dominates the total.
          </li>
          <li>
            Z-scores depend on the look-back window (1995 onward, or the series’ first month) and
            use the full sample, so history is drawn with today’s yardstick.
          </li>
          <li>
            Sleeves are policy targets; actual weights, overlays, currency hedges and valuation lags
            in private markets are not modelled.
          </li>
        </ul>
        <SourceLine sources={[ipsSource]} records={[FRED]} />
      </Panel>
      ...
      <Panel
        ...
        sub="Each factor is the average of its components’ z-scores, signed so higher always means more of the factor"
      >
                            ...
                            <tbody>
                              {f.parts.map((p) => (
                                <tr key={p.id}>
                                  ...
                                  <td className="num">{p.z === null ? '—' : signed(p.z, 2)}</td>
        ...
        <p className="panel-note">
          σ = standard deviations from the series’ own mean. A month that a monthly release has not
          yet reached carries the prior value forward for up to three months, and the component’s
          month says which. A factor is reported for a month only when at least half of its inputs
          are present. Changes are in σ. <GlossaryLink>What these terms mean</GlossaryLink>.
        </p>
        <SourceLine records={[FRED]} />
      </Panel>
      ...
      <Panel
        ...
        sub="Daily and weekly series: latest observation, change over about 30 days. Monthly series: latest month, change on the month before."
      >
        <div className="board-grid">
          {BOARD.map(({ panel, head, rows }) => (
            <section className="board-card" key={panel.key} aria-labelledby={`board-${panel.key}`}>
        ...
        <p className="panel-note">
          Levels and rescaled levels are reproduced as published (reported public); year-over-year
          and monthly changes are calculated here. A series is marked stale when its latest
          observation is older than 7 days (daily), 21 days (weekly) or 60 days after the month ends
          (monthly), measured from the retrieval date. Board values are the latest observations, so
          a daily series here can differ from the monthly average the factors use.
        </p>
        <SourceLine records={[FRED]} />
      </Panel>
      ...
      <Panel
        ...
        kicker="History"
        title={histLabel}
        sub={`Monthly, ${monthLabel(histStart)} to ${THROUGH}${histFactor ? ' — z-score, shaded band = near norm' : '; daily and weekly series as monthly averages'}`}
      >
        <div className="panel-actions" style={{ marginTop: 0, marginBottom: 8 }}>
          <label>
            Series{' '}
            <select
          ...
          {/* monthly averages of daily and weekly series are aggregates, so calculated */}
          <ClassBadge
            c={
              histFactor ||
              (histDef &&
        ...
        <details className="details-block">
          ...
          <div className="table-scroll" role="region" aria-label="History values" tabIndex={0}>
            <table className="table">
      ...
      <div id="mac-curve" className="grid-panels mt">
        <Panel
          ...
          sub="Each curve uses one date on which all five maturities were published"
        >
          {CURVES.length ? (
            <>
              <LineChart
                series={CURVES.map((c, i) => ({
                  ...
                  variant: i === 0 ? 'primary' : i === 1 ? 'secondary' : 'tertiary',
                }))}
              ...
              <div className="chart-legend curve-legend">
                {CURVES.map((c, i) => (
                  <span key={c.date}>
                    <span
                      className={`legend-line lc-${i === 0 ? 'primary' : i === 1 ? 'secondary' : 'tertiary'}`}
                    />
                    {dayLabel(c.date)}
                  </span>
                ))}
              </div>
              <div className="table-scroll" role="region" aria-label="Treasury yields" tabIndex={0}>
                <table className="table">
          ...
          <p className="panel-note">
            Maturities are evenly spaced, not to scale. Comparison dates are the last common dates
            at least 30 and 365 days before the latest.
          </p>
          <div className="kpi-provenance">
        ...
        <Panel
          ...
          sub="General mechanisms over about 30 days of observations, not a statement about LACERA’s portfolio"
        >
          ...
          <p className="panel-note">
            “Little change” thresholds: ±5 bps for yields, ±0.10 for the stress index. Changes are
            calculated from native daily or weekly observations.
          </p>
          <div className="kpi-provenance">
            <ClassBadge c="calculated" />
            <SourceLine records={[FRED]} />
      ...
      <Panel
        ...
        sub={`Retrieved from FRED on ${RETRIEVED}; each series’ redistribution terms checked on ${longDate(SNAP.termsCheckedOn)}`}
      >
        ...
        <ul className="limits">
          <li>
            A dated snapshot, not a live feed. <code>tools/fetch_macro_snapshot.py</code> pulls the
            raw observations with a FRED key kept outside the repository; the site never calls FRED
            and holds no key. Every derived figure is computed in the browser by tested code.
          </li>
          <li>
            Monthly values are FRED’s monthly averages for daily and weekly series. Year-over-year
            changes compare the same calendar month; a missing base month stays missing.
          </li>
          <li>
            Factors: equal-weighted averages of component z-scores. Credit conditions use the
            Chicago Fed credit and risk subindexes and the St. Louis Fed stress index, because the
            usual corporate-spread series may not be republished. Oil and gas enter as
            year-over-year changes.
          </li>
          <li>
            The regime read and the transmission thresholds are rules with stated assumptions; the
            portfolio lens is a proxy estimate. None is a forecast, attribution, or recommendation.
          </li>
          <li>
