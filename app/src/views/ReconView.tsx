...
import { fmtMm, Panel, Pill } from '../components/ui';
import { useDataset } from '../lib/dataset/useDataset';
import type { ReconStatus } from '../lib/finance/recon';

/**
 * Reconciliation: paired values from two sources with the variance COMPUTED on screen and the
 * threshold carried as data (tolerance_definition records) — configurable without a settings
 * store. The custodian remains the official book of record throughout.
 */

const STATUS_TONE: Record<ReconStatus, 'good' | 'warn' | 'bad' | 'neutral'> = {
  within: 'good',
  outside: 'bad',
  incomplete: 'warn',
      ...
      {recons.length === 0 ? (
        ...
        >
          ...
          <p className="panel-note">
            Sides are synthetic demo inputs; the internal-book side ties to the workbook's own EMV
            values, and one category pair is a deliberate demonstration break. Tolerances are
            absolute {recons[0]?.unit ?? '$mm'} thresholds defined in the data.
          </p>
        </Panel>
      )}
    </>
