...
export function CategoryTrend({
  ...
  return (
    <div className="x-trend">
      <p className="x-lede">
        {isTotal ? 'The Total Fund' : label} beat its benchmark in {beat} of {withBoth.length}{' '}
        months.{' '}
        {isTotal
          ? firstMv && now.mv !== null
        ...
        <div className="table-scroll" role="region" aria-label={`${label} by month`} tabIndex={0}>
          <table className="table">
            <caption>
              ...
              {isTotal ? ', market value' : ', weight and target'}
            </caption>
            <thead>
              <tr>
              ...
              {s.map((p, i) => (
                <tr key={months[i]} className={i === at ? 'trend-current' : undefined}>
                  ...
                  {reports[i] ? (
                    ...
                    <td colSpan={isTotal ? 4 : 6}>No report carries this month</td>
                  )}
                </tr>
              ))}
            </tbody>
