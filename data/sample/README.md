...
- `cio_monthly_feed_demofund.csv` — schema 1.4 CIO Monthly feed: the latest public CIO Monthly Report vintage re-expressed as `cio_monthly` rows for the DEMOFUND workspace (every row `reported_public`, citing the report page). Generated by `cd app && npm run emit:cio-sample`; round-trip tested in `app/src/lib/dataset/cioFeed.test.ts`. Import it on the Data tab, then choose "Workstation dataset" on the CIO Monthly tab.

- `cio_template_example_aug2026.csv` — the Export tab of `app/public/templates/CIO_Monthly_Template_Example.xlsx` (the CIO Monthly template filled with the latest public report), saved by desktop Excel as "CSV UTF-8" by `tools/qa_cio_template.py`. Format `cio-template-1` (`docs/cio-template.md`). Public figures only; the unit tests rebuild the published report from it. Open it with "Open a template file" on the CIO Monthly tab.
