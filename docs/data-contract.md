...
Schema version: **1.4.0** (semver; `schema_version` column on every record). The web prototype
...
## 1.4.0 additions (2026-09-07) — the CIO Monthly feed

One record type; the column set is unchanged (32 columns ⇒ schema_version 1.2+). Five period
tokens are added to `period_type`: `3M`, `YTD`, `3Y`, `5Y`, `10Y` (the monthly report's
remaining trailing periods, alongside `1M`, `FYTD`, `1Y`).

- `cio_monthly` — the Chief Investment Officer Monthly Report's figures as rows, one entity and
  one `as_of_date` (the report's data-through month end) per feed. `metric_id` is a closed
  vocabulary (V24): `return`, `benchmark_return`, `hurdle_return` (decimal, `%`, per
  `period_type`; `category_id` = `TOTAL` or a composite `GROWTH`/`CREDIT`/`RAIH`/`RRM`),
  `market_value`, `flow`, `cash` ($ millions: `unit` USD, `scale` 1000000), `weight`,
  `target_weight` (decimal, `%`), `hist_count` (`category_id` = `BIN_00` … `BIN_13`, months in
  the bin, `unit` count), `hist_stat` (`category_id` = `MEAN`/`SAA`/`SD`/`MIN`/`MAX`/`LATEST`,
  decimal). An `OTHER` category carries the unitemized overlays/other-asset line. Geography and
  the market table are not part of the feed.
- V05's natural key adds `period_type` for this type (FYTD and 1Y share a span at fiscal year
  end).
- V24 enforces the report's own identities per (entity, as_of): composite weights sum to 1
  (±0.003), composite market values sum to the total (±0.3%), a histogram has 14 bins summing to
  120, every composite `return` has a `benchmark_return` for the same period, and `TOTAL` carries
  `1M`, `FYTD` and `1Y` returns.
- Classification: `reported_public` when the rows re-express the public report (a quotation
  type, like `public_reference`); `calculated` when a workbook produces them. The CIO Monthly tab
  renders an applied feed as the "Workstation dataset" vintage beside the extracted public
  reports and shows the publication gate; the deck on the public site keeps the latest public
  report. Sample: `data/sample/cio_monthly_feed_demofund.csv` (generated by
  `npm run emit:cio-sample` from the latest public vintage; round-trip tested).
